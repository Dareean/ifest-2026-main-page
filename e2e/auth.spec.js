...
import { registerUserViaApi, verifyUserViaApi, loginViaApi, getCsrfCookie, postWithCsrf } from './helpers/seed.js'
...
test.describe('Auth Flow — Registration API Validation', () => {
  ...
  test('register with password less than 8 characters returns 422', async ({ request }) => {
    const res = await postWithCsrf(request, `${API_BASE}/auth/register`, { ...TEST_USER, email: uniqueEmail('e2e-pw-short'), password: 'Ab1', password_confirmation: 'Ab1' })
  ...
  test('register with password confirmation mismatch returns 422', async ({ request }) => {
    const res = await postWithCsrf(request, `${API_BASE}/auth/register`, { ...TEST_USER, email: uniqueEmail('e2e-pw-conf'), password: 'Password123!', password_confirmation: 'DifferentPass456!' })
    expect(res.status()).toBe(422)
    const body = await res.json()
    expect(body.errors.password).toBeDefined()
  })
  ...
  test('register with empty name returns 422', async ({ request }) => {
    const res = await postWithCsrf(request, `${API_BASE}/auth/register`, { ...TEST_USER, name: '', email: uniqueEmail('e2e-no-name'), password_confirmation: TEST_USER.password })
    expect(res.status()).toBe(422)
    const body = await res.json()
  ...
  test('register with invalid email format returns 422', async ({ request }) => {
    const res = await postWithCsrf(request, `${API_BASE}/auth/register`, { ...TEST_USER, email: 'not-an-email', password_confirmation: TEST_USER.password })
    expect(res.status()).toBe(422)
    const body = await res.json()
  ...
  test('register with phone exceeding max length returns 422', async ({ request }) => {
    const res = await postWithCsrf(request, `${API_BASE}/auth/register`, { ...TEST_USER, email: uniqueEmail('e2e-ph-long'), phone: '0'.repeat(21), password_confirmation: TEST_USER.password })
    expect(res.status()).toBe(422)
    const body = await res.json()
  ...
  test('register with institution exceeding max length returns 422', async ({ request }) => {
    const res = await postWithCsrf(request, `${API_BASE}/auth/register`, { ...TEST_USER, email: uniqueEmail('e2e-inst-l'), institution: 'A'.repeat(256), password_confirmation: TEST_USER.password })
    expect(res.status()).toBe(422)
    const body = await res.json()
  ...
  test('register with all fields empty returns 422', async ({ request }) => {
    const res = await postWithCsrf(request, `${API_BASE}/auth/register`, { name: '', email: '', password: '', password_confirmation: '' })
    expect(res.status()).toBe(422)
    const body = await res.json()
    expect(body.errors.name).toBeDefined()
    expect(body.errors.email).toBeDefined()
...
test.describe('Auth Flow — Login API Validation', () => {
  ...
  test('login returns user data', async ({ request }) => {
    const res = await postWithCsrf(request, `${API_BASE}/auth/login`, { email: userEmail, password: TEST_USER.password })
  ...
  test('login with wrong password returns 401', async ({ request }) => {
    const res = await postWithCsrf(request, `${API_BASE}/auth/login`, { email: userEmail, password: 'wrongpassword123' })
  ...
  test('login with non-existent email returns 401', async ({ request }) => {
    const res = await postWithCsrf(request, `${API_BASE}/auth/login`, { email: 'nonexistent@test.com', password: TEST_USER.password })
    expect(res.status()).toBe(401)
    const body = await res.json()
    expect(body.message).toContain('Email atau password salah')
  })
  ...
  test('login with empty password returns 422', async ({ request }) => {
    const res = await postWithCsrf(request, `${API_BASE}/auth/login`, { email: userEmail, password: '' })
    expect(res.status()).toBe(422)
    const body = await res.json()
    expect(body.errors.password).toBeDefined()
  })
...
test.describe('Auth Flow — Logout', () => {
  ...
  test('logout successfully via API returns 200', async ({ request }) => {
    await postWithCsrf(request, `${API_BASE}/auth/login`, { email: userEmail, password: TEST_USER.password })
    await getCsrfCookie()
    const res = await postWithCsrf(request, `${API_BASE}/auth/logout`, {})
    expect(res.status()).toBe(200)
  })

  test('after logout session is invalid for protected routes', async ({ request }) => {
    await postWithCsrf(request, `${API_BASE}/auth/login`, { email: userEmail, password: TEST_USER.password })
    await getCsrfCookie()
    await postWithCsrf(request, `${API_BASE}/auth/logout`, {})
  ...
  test('logout via UI redirects to login', async ({ page }) => {
    ...
    await page.goto('/dashboard/profile')
    await expect(page.getByText('Dashboard').or(page.getByText('Profil')).first()).toBeVisible({ timeout: 10000 })
...
test.describe('Auth Flow — OTP Verification', () => {
  ...
  test('send OTP returns success message', async ({ request }) => {
    await registerUserViaApi({ ...TEST_USER, email: userEmail, password_confirmation: TEST_USER.password })
    const res = await postWithCsrf(request, `${API_BASE}/auth/send-otp`, { email: userEmail })
    expect(res.status()).toBe(200)
  })

  test('send OTP with empty email returns 422', async ({ request }) => {
    const res = await postWithCsrf(request, `${API_BASE}/auth/send-otp`, { email: '' })
    expect(res.status()).toBe(422)
    const body = await res.json()
    expect(body.errors.email).toBeDefined()
  })
  ...
  test('verify OTP with wrong code returns 400', async ({ request }) => {
    const res = await postWithCsrf(request, `${API_BASE}/auth/verify-otp`, { email: userEmail, otp: '000000' })
  ...
  test('verify OTP with non-6-digit code returns 422', async ({ request }) => {
    const res = await postWithCsrf(request, `${API_BASE}/auth/verify-otp`, { email: userEmail, otp: '123' })
    expect(res.status()).toBe(422)
    const body = await res.json()
  ...
  test('verify OTP with non-existent email returns 400', async ({ request }) => {
    const res = await postWithCsrf(request, `${API_BASE}/auth/verify-otp`, { email: 'nonexistent@test.com', otp: '123456' })
    expect(res.status()).toBe(400)
    const body = await res.json()
...
test.describe('Auth Flow — Rate Limiting', () => {
  test('register endpoint is rate limited', async ({ request }) => {
    ...
    let status429 = false
    for (let i = 0; i < 20; i++) {
      const res = await postWithCsrf(request, `${API_BASE}/auth/register`, { ...TEST_USER, email: `${i}-${email}`, password_confirmation: TEST_USER.password })
      if (res.status() === 429) {
        status429 = true
        break
      }
