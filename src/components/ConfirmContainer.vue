...
watch(() => state.isOpen, (open) => {
  if (open && state.type === 'prompt') {
    nextTick(() => {
      ...
      if (promptInput.value) autoResize(promptInput.value)
    })
  }
})
</script>
            ...
            <div class="min-w-0 flex-1">
              ...
              <div v-if="state.type === 'prompt'" class="mt-3.5">
                <textarea
                  ...
                  class="w-full bg-slate-50 border-2 border-[#04000D] focus:border-accent-magenta rounded-xl px-3 py-2 text-xs font-semibold text-on-surface placeholder:text-on-surface-variant/35 focus:outline-none transition-all shadow-[2px_2px_0px_0px_#04000D] resize-none overflow-hidden"
                ></textarea>
              </div>
            </div>
          </div>
